...
const state = {
  ...
  giftBoard: 'wealth',
  chip: CHIP_VALUES[0],
  pending: {},        // bets tapped but not yet sent (coalesced)
  flushTimer: null,
  flushing: false,
  clockOffset: 0,
  phase: null,
...
function buildBoard() {
  ...
  for (const animal of state.config.animals) {
    ...
    tile.title = `${lang() === 'zh' ? animal.name : animal.en} · ${animal.payout}×`;
    tile.innerHTML = `
...
// Betting is optimistic + debounced: a tap updates the board instantly and the
// requests are coalesced into one-per-animal on a short timer. This keeps fast
// tapping smooth (no request per click, no await between taps).
function placeBet(animal) {
  if (state.phase !== 'betting') return toast(t('closed'));
  if (!state.me) return;
  const chip = state.chip;
  if (chip > state.me.coins) return toast(t('insufficient'));

  state.me.coins -= chip;
  state.myBets[animal.id] = (state.myBets[animal.id] ?? 0) + chip;
  state.pending[animal.id] = (state.pending[animal.id] ?? 0) + chip;
  renderStats();
  renderBoard();
  renderChips();

  clearTimeout(state.flushTimer);
  state.flushTimer = setTimeout(flushBets, 220);
}

async function flushBets() {
  if (state.flushing) { state.flushTimer = setTimeout(flushBets, 120); return; }
  ...
  try {
    ...
    for (const [animalId, amount] of Object.entries(batch)) {
      last = await api('/api/bet', { method: 'POST', body: { animalId, amount } });
    }
    // Trust the server's totals only if the player hasn't queued more since —
    // otherwise the optimistic view is still ahead and already correct.
    if (last && !Object.keys(state.pending).length) {
      state.me = last.user;
      state.myBets = last.myBets;
      renderStats();
      renderBoard();
      renderChips();
    }
    ...
    toast(tError(error));
    poll();   // a rejected bet (e.g. round closed) — resync to the truth
  } finally {
    state.flushing = false;
    if (Object.keys(state.pending).length) {
      clearTimeout(state.flushTimer);
      state.flushTimer = setTimeout(flushBets, 120);
    }
  }
}
...
async function poll() {
  try {
    ...
    // Don't clobber optimistic bets that are still being sent.
    const busy = state.flushing || Object.keys(state.pending).length > 0;
    if (!busy) state.myBets = data.myBets ?? {};
    els.bettors.textContent = num.format(data.bettors ?? 0);
    if (data.me && !busy) { state.me = data.me; renderStats(); renderProfile(); }
    else if (data.me) { renderProfile(); }
...
async function loadRoom() {
  try {
    const { seats, online, mySeat, messages, adminSeats } = await api('/api/room');
    state.mySeat = mySeat;
    state.adminSeats = adminSeats ?? [];
...
function renderSeats(seats, mySeat) {
  ...
  const adminSeats = state.adminSeats ?? [];
  const iAmAdmin = !!(state.me?.isAdmin || state.me?.isSuper);

  els.seats.replaceChildren(...seats.map((occ, index) => {
    const n = index + 1;
    const isAdminSeat = adminSeats.includes(n);
    ...
    // Admin-only seat badge, and a superadmin lock/unlock toggle.
    if (isAdminSeat) {
      const crown = document.createElement('span');
      crown.className = 'seat-crown';
      crown.textContent = '👑';
      crown.title = t('adminSeat');
      seat.append(crown);
    }
    if (state.me?.isSuper) {
      const tog = document.createElement('button');
      tog.className = 'seat-mode';
      tog.textContent = isAdminSeat ? '🔓' : '🔒';
      tog.title = t('adminSeatToggle');
      tog.addEventListener('click', (e) => { e.stopPropagation(); seatMode(n, !isAdminSeat); });
      seat.append(tog);
    }
    return seat;
  }));
...
async function seatMode(n, adminOnly) {
  try {
    await api('/api/admin/seat-mode', { method: 'POST', body: { seat: n, adminOnly } });
    loadRoom();
  } catch (error) { toast(tError(error)); }
}
...
function bcastText(raw) {
  try {
    ...
    if (g.type === 'wheel') {
      return t('wheelWinMsg')
        .replace('{winner}', g.winner).replace('{amount}', num.format(g.amount));
    }
    if (g.type === 'star') {
      return t('starWinMsg')
...
// A bcast is a "game" win (wheel / star travel) vs a gift broadcast.
function isGameBcast(m) {
  if (m.kind !== 'bcast') return false;
  try { const ty = JSON.parse(m.text).type; return ty === 'wheel' || ty === 'star'; }
  catch { return false; }
}
...
function renderChatFiltered() {
  ...
  const list = state.messages.filter((m) =>
    cat === 'chat' ? m.kind === 'msg'
    : cat === 'gift' ? (isGiftKind(m) && !isGameBcast(m))
    : cat === 'game' ? isGameBcast(m)
    : true)
    .slice(-20); // show only the newest 20 in each category
