...
function getSql() {
  ...
  sql = postgres(url, {
    ...
    max: 1,
    idle_timeout: 20,
    // Fail a stuck connection attempt fast so the D1 shim's retry can recover
    // instead of the request hanging until the platform timeout.
    connect_timeout: 15,
    ...
    types: {
      ...
      // SUM()/aggregate results come back as NUMERIC (oid 1700), which
      // postgres.js returns as a *string* by default — that made e.g.
      // "0" + "0" concatenate to "00" in /api/me/stats. All values here are
      // well within a safe integer, so parse NUMERIC to a Number too.
      numeric: {
        to: 1700,
        from: [1700],
        serialize: (x) => String(x),
